		...
		<div className="flex flex-col justify-between gap-2 w-full">
				...
				{chats.map((chat, index) => {
					if (chat.isPrivate) {
						return (
							<div
								key={index}
								className="flex flex-col gap-1 p-2 bg-secondary rounded-xl"
							>
								<h5 className="text-xl font-bold text-stroke">{`${chat.username} (private) :`}</h5>
								<p className="text-base font-bold pl-1 text-error">
									{chat.message}
								</p>
							</div>
						);
					...
					return (
						<div key={index} className="flex flex-col gap-1 p-2">
							...
							<p className="text-base font-bold pl-1">{chat.message}</p>
						</div>
					);
				})}
			...
			<Form {...chatForm}>
				<form
					...
					className="flex gap-3 px-4 py-3 w-full bg-main rounded-3xl"
				>
					<Input
						className="w-full bg-main border-none p-0 font-bold text-base placeholder:text-background"
						placeholder="Typing..."
						{...register("message")}
					/>
